...
export function insertJob(db: Database, job: ScheduledJob): void {
  db.prepare(
    `INSERT OR IGNORE INTO scheduled_tasks (job_id, job_type, agent_id, ticket_id, created_at_ms) VALUES (?, ?, ?, ?, ?)`
  ).run(job.jobId, job.jobType, job.agentId, job.ticketId, job.createdAtMs);
}
...
export function getActiveJobs(db: Database): ScheduledJob[] {
  return db.prepare(
    `SELECT job_id as jobId, job_type as jobType, agent_id as agentId, ticket_id as ticketId, created_at_ms as createdAtMs FROM scheduled_tasks ORDER BY created_at_ms ASC`
  ).all() as ScheduledJob[];
}
